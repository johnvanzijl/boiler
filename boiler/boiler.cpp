...
void BOILER::update() {
    ...
    if (this->mac_text_sensor_->state == "") {
        this->mac_text_sensor_->publish_state(get_mac_address());
    }

    if (this->version_text_sensor_->state == "") {
        this->version_text_sensor_->publish_state(ESPHOME_VARIANT);
    }
#endif
}

void BOILER::loop() {
    //this->mac_text_sensor_->publish_state("this is a test");
     while (available()) {
        char c = read();
...
void BOILER::dump_config(){
    ESP_LOGCONFIG(TAG, "BOILER:");
    ESP_LOGCONFIG(TAG, "  MAC Address : %s", const_cast<char *>(this->mac_.c_str()));
    ESP_LOGCONFIG(TAG, "  Firmware Version : %s", const_cast<char *>(this->version_.c_str()));
}

void BOILER::set_single_target(bool enable) {
...
void BOILER::reboot_and_read() {
    this->set_config_mode_(true);
    this->reboot_();
    this->set_timeout(1000, [this]() { this->read_all_info(); });
}

void BOILER::set_baud_rate(uint8_t state) {
    this->set_config_mode_(true);
...
void BOILER::set_regions_type(uint8_t state) {
    this->set_config_mode_(true);
    uint8_t sensor_regions_size = sizeof(sensor_regions);
...
#ifdef USE_NUMBER
void BOILER::set_region_number(int region, int coord, number::Number *n) {
    this->region_numbers_[region][coord] = n;
}
#endif

#ifdef USE_NUMBER
void BOILER::set_region(uint8_t region) {
...
void BOILER::report_position(void) {
#ifdef USE_SENSOR
    for(int i=0; i<3; i++) {
...
int16_t BOILER::transform(uint16_t data) {
    return (data>>15) == 1 ? -1 * (data&0x7FFF) : data&0x7FFF;
}


void BOILER::add_entry_point(EntryPoint *entry_point) { entry_points.emplace_back(entry_point); }

void BOILER::add_presence_region(PresenceRegion *presence_region) { presence_regions.emplace_back(presence_region); }

#ifdef USE_NUMBER
void BOILER::set_presence_timeout_number() {
...
coordinates BOILER::rotate_coordinates(double x, double y, double angle) {
...
void BOILER::send_command_(uint8_t command, const uint8_t *command_value, int command_value_len) {
...
void BOILER::set_config_mode_(bool enable) {
    uint8_t cmd = enable ? ENABLE_CONFIG : DISABLE_CONFIG;
    uint8_t cmd_value[2] = {0x01, 0x00};
    this->send_command_(cmd, enable ? cmd_value : nullptr, 2);
}

void BOILER::get_version_() { this->send_command_(READ_FIRMWARE, nullptr, 0); }

void BOILER::get_mac_() {
    uint8_t cmd_value[2] = {0x01, 0x00};
    this->send_command_(GET_MAC, cmd_value, 2);
}

void BOILER::get_regions_() { this->send_command_(GET_REGIONS, nullptr, 0); }

void BOILER::set_single_target_() { this->send_command_(SINGLE_TARGET, nullptr, 0); }

void BOILER::set_multi_target_() { this->send_command_(MULTI_TARGET, nullptr, 0); }

void BOILER::reboot_() { this->send_command_(REBOOT, nullptr, 0); }
}  // namespace boiler
}  // namespace esphome
